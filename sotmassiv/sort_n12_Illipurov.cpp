...
void sort(int* arr, int n)
{
    for (int i = 0; i < n; i++)
    {
        if (i % 2 == 0)
        {
            for (int j = 2; j < n; j += 2) //сравниваем с двойки с шагом 2
                if (arr[j - 1] > arr[j]) //если предыдущий эл-т больше текущего меняем местами
                    swap(arr[j - 1], arr[j]);
        }
        else                                                                               /// чет-нечет сортировка
        {
            for (int j = 1; j < n; j += 2) //сравниваем с еденички с шагом 2
                if (arr[j - 1] > arr[j]) //если предыдущий эл-т больше текущего меняем местами
                    swap(arr[j - 1], arr[j]);
        }
    }
